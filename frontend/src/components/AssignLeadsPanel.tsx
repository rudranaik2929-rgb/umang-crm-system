    ...
    <View>
      <View style={styles.headerRow}>
        <Text style={{ color: colors.textSecondary, fontSize: compact ? 12 : 13, flex: 1 }}>
          {unassigned} unassigned · {employees.length} employees
        </Text>
        <Pressable
        ...
        {employees.slice(0, compact ? 6 : 12).map((emp: any) => (
          <View key={emp.employee_id} style={[styles.box, { borderColor: colors.border, backgroundColor: colors.surfaceAlt }]}>
            ...
            <View style={styles.counts}>
              <Text style={{ color: colors.info, fontSize: 11, fontWeight: '700' }}>{emp.assigned_total ?? 0} assigned</Text>
              <Text style={{ color: colors.positive, fontSize: 11, fontWeight: '700' }}>{emp.assigned_completed ?? 0} done</Text>
            </View>
          </View>
        ))}
      </View>
      {unassigned > 0 ? (
        <Pressable onPress={() => router.push('/(app)/assign-leads' as any)} style={[styles.alert, { backgroundColor: '#6366F112', borderColor: '#6366F1' }]}>
          ...
          <Text style={{ color: colors.text, fontSize: 12, flex: 1 }}>{unassigned} leads need assignment</Text>
        </Pressable>
      ) : null}
    </View>
  );
