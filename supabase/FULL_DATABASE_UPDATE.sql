-- =====================================================================
-- UMANG HOMETECH LLP CRM — FULL DATABASE UPDATE (run once in Supabase)
-- =====================================================================
-- Safe to re-run (idempotent). Covers ALL current app features:
--
--   • Lead assignment (manager + Excel import auto-assign)
--   • Workflow status: Ringing, Visited, Hot, Not Interested, Follow Up
...
--
-- Excel import columns (no extra DB columns — uses fields below):
--   Lead Date → external_created_at + created_at
--   Lead Name, Phone, Locality, Configuration, Price → standard lead fields
--   Building/Project Name → notes
--   Assigne to → assigned_to + assigned_at + assigned_by + stage assigned
--
-- ORDER:
...
-- ---------------------------------------------------------------------
-- 5. INTEGRATION EVENTS (Housing sync, Meta webhooks)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 7. ROW LEVEL SECURITY (backend uses service role)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 8. NORMALIZE empty strings → null
-- ---------------------------------------------------------------------
...
-- Excel / assigned leads should be stage assigned if still new
update leads
...
-- ---------------------------------------------------------------------
-- 10. BACKFILL Housing lead_date → external_created_at
-- ---------------------------------------------------------------------
update leads
...
-- ---------------------------------------------------------------------
-- 11. BACKFILL follow_up_at from visit_followups
-- ---------------------------------------------------------------------
update leads l
...
-- 14. COLUMN COMMENTS
-- ---------------------------------------------------------------------
comment on column leads.assigned_to is 'employee_id — set by manager, bulk assign, or Excel import (Assigne to column)';
...
comment on column leads.follow_up_at is 'Next follow-up datetime — Follow Up box on dashboard';
...
-- Per-employee dashboard boxes (optional — uncomment to check):
-- select
--   e.name,
--   count(l.lead_id) filter (where l.status = 'active' and l.stage in ('new','assigned') and l.call_status is null and l.follow_up_at is null) as queue,
