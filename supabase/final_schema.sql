-- =====================================================================
-- UMANG HOMETECH LLP — CRM FINAL SCHEMA (single source of truth)
-- ---------------------------------------------------------------------
-- Run this ONCE in the Supabase SQL Editor. It is fully idempotent:
-- safe to re-run on an existing project (create ... if not exists +
-- alter ... add column if not exists). Covers the complete current flow:
--   * Telecaller + Sales Executive workspaces (site_visit role kept for
--     backward compatibility, labelled "Sales Executive" in the app)
--   * Lead "Ringing" call status
--   * Lead-level follow-ups (no site visit required)
--   * Dashboard metric buckets (counts match the opened lists)
--   * Robust employee login (password + password_hash kept in sync)
-- =====================================================================
...
insert into roles (role_id, name, description) values
  ...
  ('role_site_visit', 'site_visit', 'Legacy sales executive role (kept for compatibility)'),
  ('role_sales_executive', 'sales_executive', 'Sales executive — leads, follow-ups, conversions'),
...
-- ---------------------------------------------------------------------
-- USERS  (login + per-user sidebar access)
-- ---------------------------------------------------------------------
create table if not exists users (
  user_id text primary key,
  email text unique not null,
  password_hash text,
  password text,                 -- legacy column, kept in sync with password_hash
  name text not null,
  picture text,
...
-- ---------------------------------------------------------------------
-- VISITS + FOLLOW-UPS
-- visit_followups.visit_id is nullable so follow-ups can be scheduled
-- straight from a lead (cold / visited) without a site visit record.
-- ---------------------------------------------------------------------
...
create table if not exists bookings (
  ...
  brokerage_received numeric not null default 0,
...
-- =====================================================================
-- MIGRATION SAFETY for older databases (add any missing columns)
-- =====================================================================
...
-- Keep legacy password column in sync with password_hash so logins created
-- by a manager work on the very first attempt.
...
-- Allow lead-only follow-ups (no site visit).
...
alter table bookings add column if not exists brokerage_received numeric not null default 0;
...
-- =====================================================================
-- ROW LEVEL SECURITY (integration webhooks)
-- =====================================================================
...
-- COMPATIBILITY VIEWS
-- =====================================================================
create or replace view site_visits as select * from visits;
create or replace view site_visit_followups as select * from visit_followups;
create or replace view loan_status as select * from loans;
create or replace view activity_logs as select * from activities;
